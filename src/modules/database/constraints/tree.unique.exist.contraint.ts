...
export class UniqueTreeExistConstraint implements ValidatorConstraintInterface {
    ...
    async validate(value: any, validationArguments?: ValidationArguments) {
        ...
        // 在传入的 dto 数据中获取需要忽略的字段的值
        const ignoreValue = (validationArguments.object as any)[condition.ignore];
        // 查询条件字段的值
        const keyValue = (validationArguments.object as any)[condition.findKey];
        ...
        // 如果验证数据没有 parent ，把所有顶级分类作为验证数据，否则就把同一个父分类下的子分类作为验证数据
        const rows: any[] = await repo.find({
            where: { parent: !item.parent ? null : { id: item.parent.id } },
            withDeleted: true,
        });
        // 在忽略本身数据后，如果同级别其他数据和验证的 queryProperty 的值相同则验证失败
        return !rows.find(
...
/**
 *
 * @param params 树形数据同父节点同级别某个字段的唯一性验证
