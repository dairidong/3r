...
export class CommentService {
    ...
    async create(data: CreateCommentDto) {
        ...
        if (!isNil(parent) && parent.post.id !== data.post) {
            throw new ForbiddenException(
                'Parent comment and child comment must belong to same post',
            );
        }
    ...
    protected async getPost(id: string) {
        return !isNil(id) ? this.postRepository.findOneByOrFail({ id }) : id;
    }
    ...
    protected async getParent(current?: string, id?: string) {
        ...
        if (id !== undefined) {
            ...
            if (!parent) {
                throw new EntityNotFoundError(CommentEntity, `Parent comment ${id} doesn't exist`);
            }
        }
        return parent;
    }
